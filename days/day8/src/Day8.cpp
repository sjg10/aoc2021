...
void Day8::run(std::vector<std::string> res_files) {
    std::cout << "Day 8 run" << std::endl;
    std::ifstream is1(res_files[0]);
    if(is1.fail()) {throw std::runtime_error("Could not open file " + res_files[0]);}
    auto res = getDigits(is1);
    std::cout << "Day 8 Part 1: " <<  res.first << std::endl;
    std::cout << "Day 8 Part 2: " <<  res.second << std::endl;
}
